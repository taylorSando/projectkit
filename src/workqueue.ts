import type { Ack, Callback, CallbackArtifact } from './dispatch.js';
import {
  isReversible,
  laneForStatus,
  WORK_ITEM_LANES,
...
export const WORK_ITEM_LANE_LABELS: Record<WorkItemLane, string> = {
  triage: 'Triage',
  human: 'Human',
  agent: 'Agent',
  both: 'Review',
...
export const WORK_ITEM_SEVERITY_LABELS: Record<WorkItemSeverity, string> = {
  low: 'Low',
  normal: 'Normal',
  high: 'High',
  urgent: 'Urgent',
...
export const WORK_ITEM_STATUS_TONES: Record<WorkItemStatus, WorkItemTone> = {
  new: 'neutral',
  triaged: 'info',
  agent_running: 'info',
...
export function columnForWorkItem(item: Pick<WorkItem, 'status' | 'lane'>): WorkQueueColumnId {
  for (const column of WORK_QUEUE_COLUMNS) {
    if ((column.statuses as readonly string[]).includes(item.status)) return column.id;
  }
  const lane = laneForStatus(item.status, item.lane);
  if (lane === 'agent') return 'agent';
  if (lane === 'human' || lane === 'both') return 'review';
  if (lane === 'done') return 'done';
  return 'triage';
}

export function groupWorkItemsByColumn<T extends Pick<WorkItem, 'status' | 'lane'>>(items: readonly T[]): Record<WorkQueueColumnId, T[]> {
  const grouped: Record<WorkQueueColumnId, T[]> = { triage: [], agent: [], review: [], done: [] };
  for (const item of items) grouped[columnForWorkItem(item)].push(item);
  return grouped;
}

export function actionsForWorkItem(item: WorkItem, policy: WorkQueueActionPolicy = {}): WorkQueueAction[] {
  const actions: WorkQueueAction[] = [];
  const allow = (key: keyof WorkQueueActionPolicy, fallback = true) => policy[key] !== false && fallback;
  const disabled = (enabled: boolean, reason: string | undefined) => ({ enabled, ...(enabled ? {} : { disabled_reason: reason ?? 'not allowed' }) });

  const add = (action: Omit<WorkQueueAction, 'enabled'> & { enabled?: boolean; disabled_reason?: string }) => {
  ...
  if (item.status === 'wont_do' || item.status === 'reversed') return actions;
...
export function deriveWorkItemDispatchState(item: WorkItem, events: readonly WorkLifecycleEvent[] = []): WorkItemDispatchProjection {
  const callback = latestCallback(events);
  if (callback?.status === 'failed') return callbackProjection('failed', 'Callback failed', callback);
  if (callback?.status === 'cancelled') return callbackProjection('cancelled', 'Callback cancelled', callback);
  ...
  if (callback?.status === 'running') return callbackProjection('running', 'Agent running', callback);
...
export function extractWorkItemArtifacts(item: WorkItem, events: readonly WorkLifecycleEvent[] = []): WorkItemArtifactRef[] {
  const artifacts: WorkItemArtifactRef[] = [];
  const seen = new Set<string>();
  const push = (artifact: WorkItemArtifactRef | null) => {
    if (!artifact) return;
  ...
  for (const raw of findArtifactArrays(item.metadata)) {
    for (const value of raw) push(toArtifactRef(value, 'work_request'));
  }
  for (const event of events) {
    ...
    if (callbackArtifacts) {
      for (const value of callbackArtifacts) push(toArtifactRef(value, 'callback'));
    }
    for (const raw of findArtifactArrays(payload)) {
      for (const value of raw) push(toArtifactRef(value, event.type === 'agent.artifact_attached' ? 'callback' : 'event'));
    }
  }
  return artifacts;
}

function latestDispatchEvent(events: readonly WorkLifecycleEvent[]): WorkLifecycleEvent | null {
  return [...events]
...
function latestCallback(events: readonly WorkLifecycleEvent[]): Callback | null {
  for (const event of [...events].sort((a, b) => Date.parse(b.occurred_at) - Date.parse(a.occurred_at))) {
    ...
    if (status === 'accepted' || status === 'running' || status === 'succeeded' || status === 'failed' || status === 'cancelled') {
      return callback as unknown as Callback;
    }
  }
  return null;
}

function callbackProjection(state: WorkItemDispatchState, label: string, callback: Callback | null): WorkItemDispatchProjection {
  return {
    state,
    label,
...
function ackFromPayload(payload: Record<string, unknown> | null): Ack | undefined {
...
function findArtifactArrays(root: unknown): unknown[][] {
...
function toArtifactRef(value: unknown, source: WorkItemArtifactRef['source']): WorkItemArtifactRef | null {
  const obj = objectValue(value);
  if (!obj) return null;
  const ref =
    stringValue(obj['ref']) ??
...
function kindFromContentType(contentType: string | undefined): string | undefined {
...
export function assertWorkQueueVocabularyCoherent(): void {
  for (const status of WORK_ITEM_STATUSES) columnForWorkItem({ status, lane: laneForStatus(status, 'triage') });
  for (const lane of WORK_ITEM_LANES) {
    if (!WORK_ITEM_LANE_LABELS[lane]) throw new Error(`missing lane label: ${lane}`);
  }
  for (const severity of WORK_ITEM_SEVERITIES) {
    if (!WORK_ITEM_SEVERITY_LABELS[severity]) throw new Error(`missing severity label: ${severity}`);
  }
}
