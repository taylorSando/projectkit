...
export class GitRefSink {
    ...
    async deliver(envelope) {
        ...
        for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
            ...
            if (!r.retry)
                return { ok: false, sink: this.name, error: r.error };
    ...
    async tryAppend(line, message) {
        ...
        if (commit.code !== 0)
            return { ok: false, retry: false, error: gitErr('commit-tree', commit) };
 ...
 */
function isBenignStdinError(e) {
    return e.code === 'EPIPE' || e.code === 'ERR_STREAM_DESTROYED';
}
function signalMessage(env) {
